    ...
    for i, (x, lab) in enumerate(P):
        ...
        while ov:
            ...
            if type(ov) is mpl.lines.Line2D:
                ...
                new_y = rov.ymax
            if ov == last_ov:
                y_offset += kargs.get('offset_step',10)
                if y_offset > kargs.get('offset_step',10)*kargs.get('max_iteration',6):
                    break
            else:
                y_offset = kargs.get('offset_step',10)
            last_ov = ov
            ...
            labs[-1].set_verticalalignment("bottom")
            #ax.draw(renderer)
...
def put_Xlabels(ax, pos, labels, colors='rgb', debug=False, **kargs):
    ...
    for i, (x, lab) in enumerate(P):
        ...
        ov = __overlap(ax, labs[-1], objs+labs[:-1])
        last_ov = None
        y_offset = kargs.get('init_offset',5)
        counter = 0
        while ov:
            if counter>=kargs.get('max_iterations', 20):
                if do_debug(debug):
                    print("max iterations reached: break")
                    labs[-1].set_color('k')
                break
            counter += 1
            if do_debug(debug): print("Label \"{}\" overlap with {}".format(labs[-1].get_text(), repr(ov)))
            if type(ov) is mpl.lines.Line2D:
                new_y = __get_yextend_of_curve(ax, r, ov)[1]
            else:
                rov = ov.get_window_extent(renderer) # get the Bbox
                new_y = max(rov.ymax, rov.ymin)
            if ov == last_ov:
                y_offset += kargs.get('offset_step',10)
                if y_offset > kargs.get('offset_step',10)*kargs.get('max_offets',5):
                    if do_debug(debug):
                        print("Max offset reached: break")
                        labs[-1].set_color('k')
                    break
            else:
                y_offset = kargs.get('init_offset', 5)
            ...
            labs[-1].set_position(new_xy)
            ax.draw(renderer)
            ov = __overlap(ax, labs[-1], objs+labs[:-1], debug=dec_debug(debug))
